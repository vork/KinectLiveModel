...
bool GlowShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../BloomVertexShader.hlsl", L"../BloomPixelShader.hlsl");
	if (!result)
	{
		return false;
	}
...
bool GlowShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX& worldMatrix, XMMATRIX& viewMatrix,
	XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* regularTexture, ID3D11ShaderResourceView* glowTexture, float strenght)
{
	bool result;


	// Set the shader parameters that it will use for rendering.
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, regularTexture, glowTexture, strenght);
	if (!result)
	{
		return false;
	}
...
bool GlowShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	// Setup the description of the glow strenght constant buffer that is in the vertex shader.
...
bool GlowShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX& worldMatrix, XMMATRIX& viewMatrix,
	XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* regularTexture, ID3D11ShaderResourceView* glowTexture, float strenght)
{
	HRESULT result;
	...
	// Lock the glow constant buffer so it can be written to.
	...
	// Get a pointer to the data in the constant buffer.
	dataPtr3 = (GlowBufferType*)mappedResource.pData;

	// Copy the data into the constant buffer.
	dataPtr3->glowStrength = strenght;
	...
	deviceContext->PSSetShaderResources(0, 1, &regularTexture);
	deviceContext->PSSetShaderResources(0, 1, &glowTexture);
