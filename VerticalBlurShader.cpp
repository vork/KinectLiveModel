...
bool VerticalBlurShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../VerticalBlurShaderVs.hlsl", L"../VerticalBlurShaderPs.hlsl");
	if (!result)
	{
		return false;
	}
...
bool VerticalBlurShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, XMMATRIX& worldMatrix, XMMATRIX& viewMatrix,
...
bool VerticalBlurShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMMATRIX& worldMatrix, XMMATRIX& viewMatrix,
	XMMATRIX& projectionMatrix, ID3D11ShaderResourceView* texture, float screenHeight)
{
	HRESULT result;
